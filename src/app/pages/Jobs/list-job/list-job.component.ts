...
export class ListJobComponent implements OnInit {
  ...
  @ViewChild('jobFilter') jobFilter!: ElementRef;
...
//console.log(this.data);
    ...
    this.roleService.getActionByRoleId(localStorage.getItem("RoleId"),"Jobs").subscribe((res) => {
  ...
  applyFilter(event: Event) {
    const filterValues = {
      departmentName: this.jobFilter.nativeElement.value,
    ...
    this.dataSource.filterPredicate = (
      data: { jobName: string ;sortable: boolean;},
      filter: string
    ) => {
    ...
    if (data.jobName == ""
     ...
     // if ((data.officeName == ""&& data.contactName=="" && data.phone1=="")  )
      ...
      return (
       ( data.jobName
         .toLowerCase()
          .includes(searchText.jobName.toLowerCase())
      //     &&
...
async  search(page?: PageEvent) {
    ...
    this.jobService.searchJobs(this.searchModel).subscribe((res) => {
      this.jobModel = res.entity.entities;
      this.totalRecords = res.entity.totalRecords;
      this._detector.markForCheck()
      this.dataSource = new MatTableDataSource(this.jobModel);
...
*/
      ...
      .onClose.subscribe((res) => {
        if (res) {
          this.jobService.deleteJob(id).subscribe((result) => {
            this.jobModel = this.jobModel.filter((x) => x.id != id);
            this.search()
            this.toastrService.Delete(EntityNames.Job);
          });
  ...
  ShowEdit(id:string)
  {
    const returnUrl = this.route.snapshot.queryParams['returnUrl'] || '/Jobs/edit/'+id;
